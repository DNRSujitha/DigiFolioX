...
function HomePage({ data }) {
  const handleDownloadResume = () => {
    // Determine if education and experience should be shown
    ...
    const resumeData = `
      ...
      ${data.projects && data.projects.length > 0 ? `Projects/Repositories:
      ${data.projects.map(project => `  • ${project.title}: ${project.description}`).join('\n')}
      
      ` : ''}
      ${shouldShowEducation && data.education && data.education.length > 0 ? `Education:
      ${data.education.map(edu => `  • ${edu.degree} from ${edu.institution}${edu.year ? ` (${edu.year})` : ''}`).join('\n')}
      
      ` : ''}
      ${shouldShowExperience && data.experience && data.experience.length > 0 ? `Experience:
      ...
      ${data.certifications && data.certifications.length > 0 ? `Certifications:
      ${data.certifications.map(cert => `  • ${cert}`).join('\n')}
      
      ` : ''}
      ${"=".repeat(60)}
      Generated via DigiFolioX • Classic Portfolio
      ${"=".repeat(60)}
    `;
            ...
            <div className="space-y-12">
              ...
              <div className="relative pl-8 border-l-2 border-gold-light">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-xl text-brown-medium leading-relaxed italic">
                    "{data.about?.split('.')[0] || 'Passionate professional'}."
                  </p>
                </div>
              </div>

              {/* Key Skills - Updated to show skill.name */}
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="text-center p-6 bg-white/50 border border-gold-light/20 rounded-sm">
                  ...
                  <div className="text-sm text-brown-medium uppercase tracking-widest">{data.profession || 'Expert'}</div>
                </div>
              </div>
            ...
            <div className="relative">
              ...
              <div className="relative bg-white border-2 border-brown-light shadow-2xl p-10">
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-6">
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        <div className="font-serif text-brown-dark">{data.contactDetails?.email || 'Not provided'}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        <div className="font-serif text-brown-dark">{data.contactDetails?.mobile || 'Not provided'}</div>
                      </div>
                    </div>
                  ...
                  {/* Quick Expertise - Updated to show skill.name */}
            ...
            <button
              onClick={() => {
                ...
                if (aboutSection) {
                  ...
                  // Fallback: scroll to the next section
